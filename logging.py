...
class Logging(commands.Cog):
    ...
    async def setlogsetup(self, ctx):
        """Creates chwerreb-logging category and log channels with private permissions"""
        guild = ctx.guild
        ...
        category = discord.utils.get(guild.categories, name="chwerreb-logging")
        if not category:
            category = await guild.create_category("chwerreb-logging", overwrites=overwrites)
        ...
        await ctx.send("✅ Logging channels created privately under 'chwerreb-logging'.")

    @commands.command(name="removelogs")
    @commands.has_permissions(administrator=True)
    async def removelogs(self, ctx):
        """Removes chwerreb-logging channels and logging DB config"""
        guild = ctx.guild
        ...
        await ctx.send("🗑️ Logging channels and category 'chwerreb-logging' have been removed.")
