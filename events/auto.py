    ...
    async def send_msg_to_adder(self, guild: discord.Guild):
        async for entry in guild.audit_logs(limit=3):
            if entry.action == discord.AuditLogAction.bot_add:
                ...
                website_button = Button(label='Website', style=discord.ButtonStyle.link, url='https://example.com')
                support_button = Button(label='Support', style=discord.ButtonStyle.link, url='https://discord.gg/ZTEptRd65f')
                website_button = Button(label='KingZ FTW', style=discord.ButtonStyle.link, url=f'https://example.com')
                view = View()
