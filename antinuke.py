  ...
  async def antinuke(self, ctx, option: str = None):
    ...
    if option is None:
      ...
      if is_activated:
        ...
        setup_embed.description += "\n<:tick:1327829594954530896>| Checking chwerreb's role position for optimal configuration..."
        await setup_message.edit(embed=setup_embed)

        await asyncio.sleep(1)
        setup_embed.description += "\n<:tick:1327829594954530896> | Crafting and configuring the chwerreb Supreme role..."
        await setup_message.edit(embed=setup_embed)
        
        try:
          role = await ctx.guild.create_role(
            name="chwerreb Supreme",
            color=0x0ba7ff,
        ...
        await asyncio.sleep(1)
        setup_embed.description += "\n<:tick:1327829594954530896>| Ensuring precise placement of the chwerreb Supreme role..."
        await setup_message.edit(embed=setup_embed)
        try:
        ...
        embed.set_author(name="chwerreb Antinuke", icon_url=self.bot.user.avatar.url)

        embed.set_footer(text="Successfully Enabled Antinuke for this server | Powered by chwerreb X Development™", icon_url=self.bot.user.avatar.url)
        embed.set_thumbnail(url=self.bot.user.avatar.url)
